...
while ($row = mysqli_fetch_assoc($result)) {
    echo '<tr height="210">';
    echo '<td>' . $count . '</td>';
    echo '<td class="text-left">' . htmlspecialchars($row['observation_names']) . '</td>';
    echo '<td>' . $row['start_date'] . '</td>';
    echo '<td class="text-left">' . nl2br(htmlspecialchars($row['l1_observation'])) . '</td>';
    echo '<td>' . htmlspecialchars($row['l1_observations_by']) . '</td>';
    echo '<td class="text-left">' . nl2br(htmlspecialchars($row['l2_observation'])) . '</td>';
    echo '<td>' . htmlspecialchars($row['l2_observations_by']) . '</td>';
    
    // Image 1
    echo '<td width="210" height="210" align="center" valign="middle">';
    if (!empty($row['l1_image'])) {
        $imageUrl1 = $baseUrl . str_replace(' ', '%20', $row['l1_image']);
