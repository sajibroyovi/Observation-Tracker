...
if (!empty($_GET['search'])) {
    ...
    $where .= " AND (cr_subject LIKE ? OR impacted_area LIKE ? OR cr_meeting_attended LIKE ?)";
...
$count_sql = "SELECT COUNT(*) as total FROM cr_list" . $where;
...
// Fetch data from cr_list table with filters and limit
$sql = "SELECT * FROM cr_list " . $where . " ORDER BY serial_no DESC LIMIT $limit OFFSET $offset";
...
// Module Stats
$_stats = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total, SUM(downtime='0') as yes, SUM(downtime='1') as no, SUM(downtime='2') as fluctuation FROM cr_list")) ?: [];
$_progress = ($_stats['total'] > 0) ? round((($_stats['no'] + $_stats['fluctuation']) / $_stats['total']) * 100) : 0;
?>
        ...
        <div class="main-content">
            ...
            <div class="container-fluid ps-0 mt-3">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="view-header fw-bold mb-1"><i class="fa-solid fa-file-invoice text-purple me-2"></i> Change Request List</h1>
                        <p class="text-muted small mb-0">Monitor planned system changes, downtime, and impact zones.</p>
                    </div>
                    <div class="d-flex gap-2">
                        <?php if (canEditGlobal()): ?>
                        <button class="btn btn-primary btn-sm rounded-pill shadow-sm px-4" data-bs-toggle="modal" data-bs-target="#staticBackdrop_crlist">
                            <i class="fa-solid fa-plus me-1"></i> Add New
                        </button>
                        <?php endif; ?>
                        <a href="<?= BASE_URL ?>/modules/export/export?module=cr_list&<?= e(http_build_query($_GET)) ?>" class="btn btn-outline-success btn-sm rounded-pill shadow-sm px-3 bg-white border-0 btn-export-view"><i class="fa-solid fa-file-export me-1"></i> Export</a>
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-md-8">
                            <div class="d-flex align-items-center gap-4 flex-wrap">
                                <div class="stat-icon-wrapper bg-purple bg-opacity-10 text-purple rounded d-flex align-items-center justify-content-center shadow-sm" style="width: 48px; height: 48px;">
                                    <i class="fa-solid fa-file-invoice fa-xl"></i>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">Downtime</div>
                                    <div class="h4 fw-bold mb-0 text-danger"><?= (int)($_stats['yes'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">No Impact</div>
                                    <div class="h4 fw-bold mb-0 text-success"><?= (int)($_stats['no'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">Fluctuation</div>
                                    <div class="h4 fw-bold mb-0 text-warning"><?= (int)($_stats['fluctuation'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item pe-4">
                                    <div class="small-caps text-muted mb-1">Total CRs</div>
                        ...
                        <div class="col-md-4">
                            <div class="d-flex align-items-center justify-content-end gap-3">
                                <div class="text-end">
                                    <div class="small-caps text-muted mb-1">Stability</div>
                ...
                <div class="glass-card mb-4 p-3 py-1">
                    <form method="GET" class="row g-2 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="search" class="form-control form-control-sm border-0 bg-light" placeholder="Subject, Area, Attendee..." value="<?php echo htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        ...
                        <table class="table table-hover align-middle mb-0 dashboard-table">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $downtime_val = (string)($row["downtime"] ?? "");
                                    $downtime_badge = '<span class="status-badge bg-secondary bg-opacity-10 text-secondary fw-bold p-2 px-3 rounded-pill">' . e($downtime_val ?: "N/A") . '</span>';
                                    if ($downtime_val === "0") {
                                        $downtime_badge = '<span class="status-badge bg-success bg-opacity-10 text-success fw-bold p-2 px-3 rounded-pill">Yes</span>';
                                    } else if ($downtime_val === "1") {
                                        $downtime_badge = '<span class="status-badge bg-danger bg-opacity-10 text-danger fw-bold p-2 px-3 rounded-pill">No</span>';
                                    } else if ($downtime_val === "2") {
                                        $downtime_badge = '<span class="status-badge bg-warning bg-opacity-10 text-warning fw-bold p-2 px-3 rounded-pill">Fluctuation</span>';
                                    } else if ($downtime_val === "3") {
                                        $downtime_badge = '<span class="status-badge bg-secondary bg-opacity-10 text-secondary fw-bold p-2 px-3 rounded-pill">N/A</span>';
                                    }

                                    echo "<tr>
                                            <td class='ps-4'>" . $count++ . "</td>
                                            <td class='fw-bold' style='max-width: 200px;'>" . e($row["cr_subject"] ?? "N/A") . "</td>
                                            <td class='small'>" . e($row["impacted_area"] ?? "N/A") . "</td>
                                            <td>
                                                <div class='small fw-bold'>" . ($row["cr_start_time"] ? date('d M, Y', strtotime($row['cr_start_time'])) : 'N/A') . "</div>
                                                <div class='text-muted font-monospace' style='font-size: 0.7rem;'>" . ($row["cr_start_time"] ? date('H:i', strtotime($row['cr_start_time'])) : '') . "</div>
                                            </td>
                                            <td>
                                                <div class='small fw-bold'>" . ($row["cr_end_time"] ? date('d M, Y', strtotime($row['cr_end_time'])) : 'N/A') . "</div>
                                                <div class='text-muted font-monospace' style='font-size: 0.7rem;'>" . ($row["cr_end_time"] ? date('H:i', strtotime($row['cr_end_time'])) : '') . "</div>
                                            </td>
                                            <td>" . $downtime_badge . "</td>
                                            <td><span class='badge bg-light text-dark font-italic fw-normal p-2'>" . e($row["cr_meeting_attended"] ?? "N/A") . "</span></td>
                                            <td>
