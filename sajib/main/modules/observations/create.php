    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($_FILES['l1_images']['error'][$i] == 0) {
                    ...
                    if ($check !== false) {
                        ...
                        if ($_FILES["l1_images"]["size"][$i] > $max_size) {
                            ...
                            header('Location: ' . BASE_URL . '/?status=error&msg=' . urlencode("Error: File " . $_FILES["l1_images"]["name"][$i] . " exceeds the 5MB limit."));
                            exit;
                        }
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                header('Location: ' . BASE_URL . '/?status=success&msg=' . urlencode("Observation record inserted successfully"));
            } else {
