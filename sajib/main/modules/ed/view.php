...
if (!empty($_GET['search'])) {
    ...
    $where .= " AND (service_name LIKE ? OR reference LIKE ? OR action_taken_by LIKE ?)";
...
$count_sql = "SELECT COUNT(*) as total FROM enable_disable" . $where;
...
// Fetch data from enable_disable table with filters and limit
$sql = "SELECT * FROM enable_disable " . $where . " ORDER BY serial_no DESC LIMIT $limit OFFSET $offset";
...
// Module Stats
$_stats = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total, SUM(action_taken='0') as enabled, SUM(action_taken='1') as disabled, SUM(action_taken='2') as hidden FROM enable_disable")) ?: [];
$_progress = ($_stats['total'] > 0) ? round(($_stats['enabled'] / $_stats['total']) * 100) : 0;
?>
...
<head>
    ...
    <title>View Enable/Disable Service</title>
        ...
        <div class="main-content">
            ...
            <div class="container-fluid ps-0 mt-3">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="view-header fw-bold mb-1"><i class="fa-solid fa-toggle-on text-primary me-2"></i> Enable/Disable Records</h1>
                        <p class="text-muted small mb-0">Monitor and manage service reachability status logs.</p>
                    </div>
                    <div class="d-flex gap-2">
                        <?php if (canEditGlobal()): ?>
                        <button class="btn btn-primary btn-sm rounded-pill shadow-sm px-4" data-bs-toggle="modal" data-bs-target="#staticBackdrop_ed">
                            <i class="fa-solid fa-plus me-1"></i> Add New
                        </button>
                        <?php endif; ?>
                        <a href="<?= BASE_URL ?>/modules/export/export?module=enable_disable&<?= e(http_build_query($_GET)) ?>" class="btn btn-outline-success btn-sm rounded-pill shadow-sm px-3 bg-white border-0 btn-export-view"><i class="fa-solid fa-file-export me-1"></i> Export</a>
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-md-8">
                            <div class="d-flex align-items-center gap-4 flex-wrap">
                                <div class="stat-icon-wrapper bg-primary bg-opacity-10 text-primary rounded d-flex align-items-center justify-content-center shadow-sm" style="width: 48px; height: 48px;">
                                    <i class="fa-solid fa-toggle-on fa-xl"></i>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">Enabled</div>
                                    <div class="h4 fw-bold mb-0 text-success"><?= (int)($_stats['enabled'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">Disabled</div>
                                    <div class="h4 fw-bold mb-0 text-danger"><?= (int)($_stats['disabled'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item border-end pe-4">
                                    <div class="small-caps text-muted mb-1">Hidden</div>
                                    <div class="h4 fw-bold mb-0 text-warning"><?= (int)($_stats['hidden'] ?? 0) ?></div>
                                </div>
                                <div class="stat-item pe-4">
                                    <div class="small-caps text-muted mb-1">Total Logs</div>
                        ...
                        <div class="col-md-4">
                            <div class="d-flex align-items-center justify-content-end gap-3">
                                <div class="text-end">
                                    <div class="small-caps text-muted mb-1">Availability</div>
                    ...
                    <form method="GET" class="row g-2 align-items-end">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="status_filter" class="form-select form-select-sm border-0 bg-light">
                                ...
                                <option value="0" <?php echo (isset($_GET['status_filter']) && $_GET['status_filter'] === '0') ? 'selected' : ''; ?>>Enabled</option>
                                <option value="1" <?php echo (isset($_GET['status_filter']) && $_GET['status_filter'] === '1') ? 'selected' : ''; ?>>Disabled</option>
                                <option value="2" <?php echo (isset($_GET['status_filter']) && $_GET['status_filter'] === '2') ? 'selected' : ''; ?>>Hidden</option>
                                <option value="3" <?php echo (isset($_GET['status_filter']) && $_GET['status_filter'] === '3') ? 'selected' : ''; ?>>Unhidden</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <label class="small text-muted fw-bold">SEARCH</label>
                            <input type="text" name="search" class="form-control form-control-sm border-0 bg-light" placeholder="Service, Ref, User..." value="<?php echo htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        ...
                        <table class="table table-hover align-middle mb-0 dashboard-table">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $action_val = (string)($row["action_taken"] ?? "");
                                    $action_badge = '<span class="status-badge bg-secondary bg-opacity-10 text-secondary fw-bold p-2 px-3 rounded-pill">' . e($action_val ?: "N/A") . '</span>';
                                    if ($action_val === "0") {
                                        $action_badge = '<span class="status-badge bg-success bg-opacity-10 text-success fw-bold p-2 px-3 rounded-pill">Enabled</span>';
                                    } else if ($action_val === "1") {
                                        $action_badge = '<span class="status-badge bg-danger bg-opacity-10 text-danger fw-bold p-2 px-3 rounded-pill">Disabled</span>';
                                    } else if ($action_val === "2") {
                                        $action_badge = '<span class="status-badge bg-warning bg-opacity-10 text-warning fw-bold p-2 px-3 rounded-pill">Hidden</span>';
                                    } else if ($action_val === "3") {
                                        $action_badge = '<span class="status-badge bg-info bg-opacity-10 text-info fw-bold p-2 px-3 rounded-pill">Unhidden</span>';
                                    }
                                    echo "<tr>
                                            <td class='ps-4'>" . $count++ . "</td>
                                            <td class='fw-bold'>" . e($row["service_name"] ?? "N/A") . "</td>
                                            <td>
                                                <div class='small fw-bold'>" . date('d M, Y', strtotime($row['action_date'])) . "</div>
                                                <div class='text-muted' style='font-size: 0.75rem;'>" . date('g:i A', strtotime($row['action_date'])) . "</div>
                                            </td>
                                            <td>" . $action_badge . "</td>
                                            <td>" . e($row["action_taken_by"] ?? "N/A") . "</td>
                                            <td><span class='badge bg-light text-dark font-monospace fw-normal p-2'>" . e($row["reference"] ?? "None") . "</span></td>
                                            <td>
