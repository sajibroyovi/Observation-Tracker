...
if (!empty($_GET['search'])) {
    ...
    $where .= " AND (campaign_name LIKE ? OR description LIKE ?)";
...
$count_sql = "SELECT COUNT(*) as total FROM campaign" . $where;
...
// Fetch data from campaign table with filters and limit
$sql = "SELECT * FROM campaign " . $where . " ORDER BY serial_no DESC LIMIT $limit OFFSET $offset";
        ...
        <div class="main-content">
            ...
            <div class="container-fluid ps-0 mt-3">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="view-header fw-bold mb-1"><i class="fa-solid fa-bullhorn text-pink me-2"></i> Campaign Records</h1>
                        <p class="text-muted small mb-0">Monitor active promotions and seasonal marketing efforts.</p>
                    </div>
                    <div class="d-flex gap-2">
                        <?php if (canEditGlobal()): ?>
                        <button class="btn btn-primary btn-sm rounded-pill shadow-sm px-4" data-bs-toggle="modal" data-bs-target="#staticBackdrop_campaign">
                            <i class="fa-solid fa-plus me-1"></i> Add New
                        </button>
                        <?php endif; ?>
                        <a href="<?= BASE_URL ?>/modules/export/export.php?module=campaign&<?php echo http_build_query($_GET); ?>" class="btn btn-outline-success btn-sm rounded-pill shadow-sm px-3 bg-white border-0 btn-export-view"><i class="fa-solid fa-file-export me-1"></i> Export</a>
                ...
                <div class="glass-card mb-4 p-3 py-1">
                    <form method="GET" class="row g-2 align-items-end">
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="text" name="search" class="form-control form-control-sm border-0 bg-light" placeholder="Name, Desc..." value="<?php echo e($_GET['search'] ?? ''); ?>">
                        </div>
                        <div class="col-md-2 d-flex gap-2">
                            ...
                            <a href="view_campaign.php" class="btn btn-outline-secondary btn-sm rounded-pill px-3"><i class="fa-solid fa-rotate me-1"></i> Reset</a>
                        </div>
                    </form>
                </div>
                        ...
                        <table class="table table-hover align-middle mb-0 dashboard-table">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    if ($status == 'active') {
                                        $status_badge = '<span class="status-badge bg-success bg-opacity-10 text-success fw-bold p-2 px-3 rounded-pill"><i class="fa-solid fa-circle-play me-1"></i> Active</span>';
                                    } else if ($status == 'inactive') {
                                        $status_badge = '<span class="status-badge bg-secondary bg-opacity-10 text-secondary fw-bold p-2 px-3 rounded-pill"><i class="fa-solid fa-circle-pause me-1"></i> Inactive</span>';
                                    } else if ($status == 'completed') {
                                        $status_badge = '<span class="status-badge bg-primary bg-opacity-10 text-primary fw-bold p-2 px-3 rounded-pill"><i class="fa-solid fa-circle-check me-1"></i> Completed</span>';
                                    }

                                    echo "<tr>
                                            <td class='ps-4'>" . $count++ . "</td>
                                            <td class='fw-bold'>" . e($row["campaign_name"] ?? "N/A") . "</td>
